...
function wrap(callback, name) {
  ...
  function _trycatchNewCallback() {
    ...
    if (isLST) {
      ...
      } else if (trycatch.currentStack !== false) {
        trycatch.currentStack = stack
      }
    }
  ...
  if (options['long-stack-traces']) {
    if (err.__trycatchcurrentStack__) {
      ...
      if (!trycatch.sameTick && !isStrictMode) {
        ...
        currentStack = trycatch.currentStack
      }
    }
...
/*
Account when err.stack is accessed asynchronously
causing the wrong trycatch.currentStack to be used
But we can't set it on the error at creation time
So we have to somehow link the original stack frames to the currentStackFrame at that time.... we used to use frame.getFunction().currentStack
*/
